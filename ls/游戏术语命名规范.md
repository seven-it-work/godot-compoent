...
| 手牌 | Hand | Player.hand | 玩家已购买但未放置到战场上的随从列表，最大7个位置。 |
...
- 等待机制：法术列表满时，生成的塑造法术进入`pendingSpells`队列，当法术列表有空位时自动添加
- 状态标记：随从通过`hasGrantedShapingSpell`属性标记是否已生成塑造法术，回合开始时重置为false
- 临时标记：通过`Spell.isTemporary`属性标记为临时法术，用于回合结束时筛选销毁
- 代码实现：`Player.addSpell()`（添加法术）、`Player.onTurnEndSpells()`（处理临时法术）、`Player.tryAddPendingSpells()`（添加等待队列法术）
