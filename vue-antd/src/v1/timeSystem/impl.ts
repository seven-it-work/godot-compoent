 ...
 */
export function autoRegisterTimeFlowHandler<
...
>(target: T): T {
  ...
  const decorated = function (...args: any[]) {
    ...
    // 将实例注册到时间流逝处理器管理器
    TimeFlowHandlerManagerImpl.getInstance().registerHandler(
      instance as unknown as TimeFlowHandler
    );
