 ...
 * @returns 姓名数组
 */
export function randomCultivatorName(
  gender: "male" | "female",
  count: number = 1,
  ...
  if (!allowDuplicate) {
    ...
    if (count > maxCombinations) {
      throw new Error(
        `无法生成 ${count} 个唯一名字。最大可能的组合数为 ${maxCombinations}。`
      );
    }
  }
  ...
  if (allowDuplicate) {
    ...
    const actualOneCharNames = Math.min(
      totalOneCharNames,
      maxOneCharCombinations
    );
    const actualTwoCharNames = Math.min(
      count - actualOneCharNames,
      maxTwoCharCombinations
    );
