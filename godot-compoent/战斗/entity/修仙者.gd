...
class BaseCultivator:
	...
	var _level:BaseValue.GrowthValue=BaseValue.GrowthValue.new({"value":1,"min_growth":1,"max_growth":1,"growth_factor":1})
	# 修仙者生命值（使用RangeGrowth跟踪最小值/最大值范围）
	var _health:BaseValue.RangeGrowth=BaseValue.RangeGrowth.new({"value":100,"max":100,"min":0,"min_growth":3,"max_growth":7,"growth_factor":5,"should_only_grow_max_value":true})
	# 修仙者灵气值（使用RangeGrowth跟踪最小值/最大值范围）
	var _mana:BaseValue.RangeGrowth=BaseValue.RangeGrowth.new({"value":0,"max":100,"min":0,"min_growth":5,"max_growth":10,"growth_factor":8,"should_only_grow_max_value":true})
	# 修仙者攻击力（使用RandomGrowth进行可变伤害计算）
	var _attack:BaseValue.RandomGrowth=BaseValue.RandomGrowth.new({"min":8,"max":15,"min_growth":1,"max_growth":3,"growth_factor":2})
	# 修仙者防御力（使用RandomGrowth进行可变伤害减免）
	var _defense:BaseValue.RandomGrowth=BaseValue.RandomGrowth.new({"min":5,"max":10,"min_growth":1,"max_growth":2,"growth_factor":1.5})
	#region 外部方法
	## 是否能够升级
